...
arg_list <- list(prior_rho_min = 1, # 
                 prior_rho_prob = 0.01, # Want p(rho < 3) = 0.0001 -> p(log_kappa < -0.058) = 0.0001
                 ...
                 prior_iideffect_sd_prob = 0.001, # Made this stronger because too much iid.
                 prior_iideffect_pr_sd_max = 0.05,
                 prior_iideffect_pr_sd_prob = 0.001,
...
save(cv1_output3, file = 'model_outputs/mdg_joint_cv_1_testpriors.RData')
...
cv1_output3 <- run_cv(data_cv1_mdg, mesh_mdg, its = 1000, 
                      ...
                      parallel_delay = delay, cores = 5, drop_covs = 9)
obspred_map(data_cv1_mdg, cv1_output3, column = FALSE)
ggsave('figs/mdg_both_obspred_map_oldpriors.png')
obspred_map(data_cv1_mdg, cv1_output3, trans = 'log10', column = FALSE)
ggsave('figs/mdg_both_obspred_map_log_oldpriors.png')
autoplot(cv1_output3, type = 'obs_preds', CI = TRUE)
ggsave('figs/mdg_both_obspred_oldpriors.png')

save(cv1_output3, file = 'model_outputs/mdg_joint_cv_1_oldpriors.RData')
...
iidold <- sapply(cv1_output3_old$models, function(x) x$model$opt$par['iideffect_log_tau'])
iidnew <- sapply(cv1_output3_new$models, function(x) x$model$opt$par['iideffect_log_tau'])
png('figs/robust_iid_mdg.pdf')
plot(iidold, iidnew, main = 'iid log sd')
abline(0,1)
dev.off()

spold <- sapply(cv1_output3_old$models, function(x) x$model$opt$par['log_sigma'])
spnew <- sapply(cv1_output3_new$models, function(x) x$model$opt$par['log_sigma'])
png('figs/robust_space_mdg.pdf')
plot(spold, spnew, main = 'spatial log sd')
abline(0,1)
dev.off()


rangeold <- sapply(cv1_output3_old$models, function(x) x$model$opt$par['log_rho'])
rangenew <- sapply(cv1_output3_new$models, function(x) x$model$opt$par['log_rho'])
png('figs/robust_range_mdg.pdf')
plot(rangeold, rangenew, main = 'spatial range')
abline(0,1)
dev.off()
